...
CREATE TABLE Seat (
    ...
    FOREIGN KEY (hId, tId) REFERENCES Hall(hId, tId) ON DELETE CASCADE ON UPDATE CASCADE
);
...
CREATE TABLE Payment (
    ...
    ticketNo INT,
    Price DECIMAL(9, 2),
    --Amount DECIMAL(9, 2) AS ticketNo * Price,
    Method VARCHAR(10),
    PRIMARY KEY (pId)
);

ALTER  TABLE Payment ADD Amount  AS ticketNo * Price ;
...
CREATE TABLE BookingTicket (
    ...
    FOREIGN KEY (shId) REFERENCES MovieShow(shId) ON DELETE NO ACTION ON UPDATE NO ACTION,
    FOREIGN KEY (sId, hId, tId) REFERENCES Seat(sId, hId, tId) ON DELETE NO ACTION ON UPDATE NO ACTION,
...
INSERT INTO Payment (pId, ticketNo, Price, Method)
